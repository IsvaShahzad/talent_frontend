...
*/
  ...
  const CVUpload = ({ onUpload }) => {
    const handleFileChange = (e) => {
      ...
      setSelectedFiles(files); // store in state
    ...
    return (
      <div>
        <CFormInput
          ...
          multiple
          accept=".pdf"
          value={''} // leave empty to allow re-upload
          onChange={handleFileChange}
        />
        <p style={{ fontSize: '0.85rem', marginTop: '0.5rem' }}>
          {selectedFiles.length > 0
            ? `Selected file(s): ${selectedFiles.map(f => f.name).join(', ')}`
  ...
  const handleCVUpload = async (files) => {
    ...
    setUploadingCV(true); // show overlay immediately
    setUploadProgress(0);
    //  await new Promise(r => setTimeout(r, 9000))
    try {

      const data = await uploadFiles(files); // ✅ waits until upload completes
      ...
      setUploadingCV(false); // hide overlay after upload
